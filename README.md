...
- Duplicate entries appeared across multiple scrape dates. If the contact name and LinkedIn URL are the same across entries, only the latest scraped one is kept. When the URL is invalid, and contact and company names are both the same across entries, assume they refer to the same person, and only the latest scraped data is kept.
...
Exact title matching was applied at first, but it's rigid, and fewer results were generated. 
...
If there are multiple senior role contacts from the same company, the one with the highest rank will be kept. If there is a tie, the most recent scraped_at value was used.

## Limitations
Some company names are missing, and a few can be inferred from headlines, but that doesn't apply to all cases. Although it worked for this file, maybe it shouldn't be treated as certain and could be further improved.
